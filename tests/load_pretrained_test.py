...
GOOGLE_BERT_PATH = os.environ["GOOGLE_BERT_PATH"]
GOOGLE_ALBERT_PATH = os.environ["GOOGLE_ALBERT_PATH"]
PYTORCH_MODEL_PATH = os.environ["PYTORCH_MODEL_PATH"]
CHINESE_BERT_PATH = os.environ["CHINESE_BERT_PATH"]


class LoadPretrainedModelTest(tf.test.TestCase):
    def _build_bert_inputs(self):
        vocab_path = os.path.join(GOOGLE_BERT_PATH, "uncased_L-6_H-768_A-12", "vocab.txt")
    ...
    def _build_model_from_transformers(self):
        ...
        model = TFBertModel.from_pretrained(
            os.path.join(PYTORCH_MODEL_PATH, "bert_uncased_L-6_H-768_A-12-pytorch"), from_pt=True
        )
        return model

    def _build_model(self):
        model = Bert.from_pretrained(
            os.path.join(GOOGLE_BERT_PATH, "uncased_L-6_H-768_A-12"), return_states=True, verbose=False
        )
        return model
    ...
    def _build_albert_from_transformers(self):
        ...
        model = TFAlbertModel.from_pretrained(os.path.join(PYTORCH_MODEL_PATH, "albert_base_zh_pytorch"), from_pt=True)
        return model

    def test_albert_compare_with_transformers(self):
        model = Albert.from_pretrained(os.path.join(GOOGLE_ALBERT_PATH, "albert-base-zh"))
        ...
        def _compare_mapping_in_output():
            ...
            b_output = b_layer(
                transformer_model.get_layer("albert").embeddings(
                    input_ids=input_ids, token_type_ids=segment_ids, position_ids=None, training=False
                )
            )
            self.assertAllClose(a_output, b_output)
        ...
        def _compare_final_outputs():
            ...
            self.assertAllClose(a_pooled_output, b_pooled_output, rtol=0.004, atol=1e-5)
    ...
    def test_load_pretrained_bert(self):
        ...
        # skip weights
        model = Bert.from_pretrained(
            os.path.join(CHINESE_BERT_PATH, model_paths[0]), skip_token_embedding=True, skip_pooler=True, verbose=False
        )
        model.summary()
        self._do_predict(model)
    ...
    def test_load_pretrained_albert(self):
        ...
        for p in model_paths:
            model = Albert.from_pretrained(os.path.join(GOOGLE_ALBERT_PATH, p))
            model.summary()
            self._do_predict(model)

        model = Albert.from_pretrained(
            os.path.join(GOOGLE_ALBERT_PATH, model_paths[0]),
    ...
    def test_albert_classify(self):
        def _build_albert_model(trainable=True):
            ...
            albert = Albert.from_pretrained(os.path.join(GOOGLE_ALBERT_PATH, "albert-base-zh"))
